...
<table class="overview__table">
  ...
  <tbody>
    {%- for s in site.data.definitions.sdlc_stages -%}
...
<table class="overview__table">
  ...
  <tbody>
    {%- for a in site.data.definitions.supply_chain_actors -%}
...
<ul class="tac-summary__list">
  ...
  {%- for t in site.data.definitions.slsa_threats -%}
    {%- for r in records -%}
      {%- for c in r.threats -%}
        {%- if c.id == t.id and c.status == "needs-review" -%}
          {%- assign threat_nr_count = threat_nr_count | plus: 1 -%}
          <li>
            <code class="threat__code">{{ t.code }}</code> {{ t.label }} — mitigation: <strong>{{ r.name }}</strong>
